  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        const allowedFields = ['machine_id', 'start_time', 'stop_time', 'status', 'due_date', 'quantity', 'priority', 'product_name', 'environment', 'customer_info', 'specifications', 'scheduled_start_date', 'scheduled_start_shift', 'scheduled_end_date', 'scheduled_end_shift', 'notes'];
        const updateFields = [];
        const values = [];
        let paramCounter = 1;
...
app.get('/api/labor-planner/machines', authenticateToken, async (req, res) => {
  try {
    const client = await pool.connect();
    try {
      ...
      // Get machines that have labor assignments OR scheduled orders for the specified date
      const query = `
        SELECT DISTINCT
          m.id as machine_id,
          m.name as machine_name,
        ...
        LEFT JOIN production_orders po ON m.id = po.machine_id 
          AND (
            -- Orders scheduled for this specific date
            ...
            -- Or active orders that are currently running (only if they have scheduling that includes this date)
    ...
    console.error('Error fetching machines for labor planning:', error);
    res.status(500).json({ error: 'Failed to fetch machines for labor planning', details: error.message });
  }
});
