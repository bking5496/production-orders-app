-- Enhanced Downtime Tracking Database Schema for PostgreSQL
...
CREATE TABLE IF NOT EXISTS downtime_categories (
    ...
    description TEXT,
    color_code VARCHAR(7) DEFAULT '#ff6b6b',
...
INSERT INTO downtime_categories (category_code, category_name, description, color_code, requires_approval, priority_level) VALUES
...
CREATE TABLE production_stops_enhanced (
    ...
    -- Status and Impact
    status VARCHAR(20) DEFAULT 'active' CHECK(status IN ('active', 'investigating', 'resolving', 'resolved', 'approved')),
    severity VARCHAR(20) DEFAULT 'medium' CHECK(severity IN ('low', 'medium', 'high', 'critical')),
    production_impact VARCHAR(20) CHECK(production_impact IN ('none', 'minor', 'moderate', 'major', 'severe')),
...
CREATE TABLE IF NOT EXISTS downtime_alerts (
    id SERIAL PRIMARY KEY,
    downtime_id INTEGER NOT NULL REFERENCES production_stops_enhanced(id),
    alert_type VARCHAR(50) NOT NULL CHECK(alert_type IN ('new_downtime', 'duration_exceeded', 'critical_failure', 'approval_required')),
    severity VARCHAR(20) DEFAULT 'medium' CHECK(severity IN ('low', 'medium', 'high', 'critical')),
    message TEXT NOT NULL,
    recipient_role VARCHAR(50), -- Target role for notification
...
CREATE TABLE IF NOT EXISTS maintenance_schedules (
    ...
    maintenance_type VARCHAR(20) NOT NULL CHECK(maintenance_type IN ('daily', 'weekly', 'monthly', 'quarterly', 'annual', 'hours_based', 'cycles_based')),
    task_description TEXT NOT NULL,
    frequency_value INTEGER NOT NULL, -- e.g., 7 for weekly
    frequency_unit VARCHAR(10) NOT NULL CHECK(frequency_unit IN ('days', 'weeks', 'months', 'hours', 'cycles')),
    estimated_duration INTEGER NOT NULL, -- Minutes
    last_performed TIMESTAMP,
    next_due TIMESTAMP NOT NULL,
    assigned_technician INTEGER REFERENCES users(id),
    priority INTEGER DEFAULT 3,
    status VARCHAR(20) DEFAULT 'scheduled' CHECK(status IN ('scheduled', 'due', 'overdue', 'in_progress', 'completed')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS downtime_costs (
    id SERIAL PRIMARY KEY,
    downtime_id INTEGER NOT NULL REFERENCES production_stops_enhanced(id),
    cost_type VARCHAR(20) NOT NULL CHECK(cost_type IN ('labor', 'materials', 'utilities', 'lost_production', 'overtime', 'contractor', 'parts')),
...
-- Insert some sample maintenance schedules (only if machine exists)
INSERT INTO maintenance_schedules (machine_id, maintenance_type, task_description, frequency_value, frequency_unit, estimated_duration, next_due)
SELECT 1, 'daily', 'Daily cleaning and inspection', 1, 'days', 30, CURRENT_TIMESTAMP + INTERVAL '1 day'
WHERE EXISTS (SELECT 1 FROM machines WHERE id = 1)
ON CONFLICT DO NOTHING;

INSERT INTO maintenance_schedules (machine_id, maintenance_type, task_description, frequency_value, frequency_unit, estimated_duration, next_due)
SELECT 1, 'weekly', 'Lubrication and belt tension check', 7, 'days', 60, CURRENT_TIMESTAMP + INTERVAL '3 days'
WHERE EXISTS (SELECT 1 FROM machines WHERE id = 1)
ON CONFLICT DO NOTHING;

INSERT INTO maintenance_schedules (machine_id, maintenance_type, task_description, frequency_value, frequency_unit, estimated_duration, next_due)
SELECT 2, 'monthly', 'Comprehensive equipment inspection', 1, 'months', 120, CURRENT_TIMESTAMP + INTERVAL '15 days'
WHERE EXISTS (SELECT 1 FROM machines WHERE id = 2)
ON CONFLICT DO NOTHING;

INSERT INTO maintenance_schedules (machine_id, maintenance_type, task_description, frequency_value, frequency_unit, estimated_duration, next_due)
SELECT 3, 'quarterly', 'Major component replacement', 3, 'months', 240, CURRENT_TIMESTAMP + INTERVAL '30 days'
WHERE EXISTS (SELECT 1 FROM machines WHERE id = 3)
ON CONFLICT DO NOTHING;
